...
from user.views import register, user_login, user_logout,user_home, user_answer, user_question, reset_password, \
    get_check_code \
    , edit_profile, update_image, change_password,  \
     user_collect_answer, user_follow_topic, user_follow_question, \
    user_follow_user \
    , user_followed_by_user, user_topic_answer, follow_user, delete_answer

# from user.views import register, user_login, user_logout, user_confirm, \
#     resend_confirm_email, user_home, user_answer, user_question, reset_password, \
#     get_check_code \
...
from known.views import index, question_detail, answer_detail, explore, \
    topic_list, topic_detail, add_follow_answer, cancel_follow_answer, \
    comment_answer, follow_question \
    , collect_answer, follow_topic, ask_question, question_list, \
    answer_question, topic_question, topic_answerer, follow_topic_user, \
    explore_recommend, follow_question_user \
    , search, upload, download, data

urlpatterns = [
    path('admin/', admin.site.urls),
    path('', index, name='index'),

    path('question/detail/<int:question_id>/', question_detail,
    ...
    path('question/detail/<int:question_id>/follow_question_user/',
         follow_question_user, name='follow_question_user'),

    path('topic_list/', topic_list, name='topic_list'),
    path('topic_detail/<int:topic_id>/', topic_detail, name='topic_detail'),
    path('topic_detail/<int:topic_id>/follow_topic/', follow_topic,
         name='follow_topic'),
    path('topic_detail/<int:topic_id>/question/', topic_question,
         name='topic_question'),
    path('topic_detail/<int:topic_id>/answerer/', topic_answerer,
         name='topic_answerer'),
    path('topic_detail/<int:topic_id>/follow_topic_user/', follow_topic_user,
         name='follow_topic_user'),

    # path('answer/detail/<int:answer_id>/add_follow_answer/', add_follow_answer, name='add_follow_answer'),
    path('answer/add_follow_answer/', add_follow_answer,
         name='add_follow_answer'),
    # path('answer/detail/<int:answer_id>/cancel_follow_answer/', cancel_follow_answer, name='cancel_follow_answer'),
    path('answer/cancel_follow_answer/', cancel_follow_answer,
         name='cancel_follow_answer'),
    path('answer/detail/<int:answer_id>/comment_answer/', comment_answer,
         name='comment_answer'),
    ...
    path('question/detail/<int:question_id>/answer_question/', answer_question,
         name='answer_question'),

    path('home/', data, name='data'),
    path('^upload/$', upload, name='upload'),
    path('^download/(?P<filename>.+)$', download, name='download'),
    ...
    path('user/<int:user_id>/followed_by_user/', user_followed_by_user,
         name='user_followed_by_user'),
    path('user/<int:user_id>/topic_answer/<int:topic_id>/', user_topic_answer,
         name='user_topic_answer'),
...
# 全局404错误
handler404 = 'known.views.custom_page_not_found'

# 全局500错误
handler500 = 'known.views.server_error'
